...
| `maxVoices`        | `32`     | Maximum simultaneous voices                    |
| `startOptions`     | defaults | Options passed to `start()` when auto-starting |

Create a new `Audio` instance to change `sampleRate`, `playbackChannels`, or `maxVoices`.
...
| `sound.unload()`                | Free a loaded sound and stop voices using it                  |
...
`sound.play()` accepts `{ volume, pan, loop, group }`. Native playback clamps `volume` to `0..4` and `pan` to `-1..1`. Configure the voice slot limit with `Audio.create({ maxVoices })`. `AudioSound`, `AudioVoice`, and `AudioGroup` are object handles; pass those objects back into audio methods instead of numeric IDs.
