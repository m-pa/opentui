...
pub fn brightness(self: anytype, triplets: []const f32) void {
    if (triplets.len < 3) return;
    ...
    while (i < len) : (i += 3) {
        const x_f = triplets[i];
        const y_f = triplets[i + 1];
        const brightness_factor = triplets[i + 2];

        if (!math.isFinite(x_f) or !math.isFinite(y_f) or !math.isFinite(brightness_factor)) continue;
        if (x_f < 0 or y_f < 0 or x_f >= width_f or y_f >= height_f) continue;
